const countries_data = [
  ...
  let popBtn = document.querySelector("#populationBtn");
  let langBtn = document.querySelector("#languagesBtn");
  let popSection = document.querySelectorAll(".popSection");
  let langSection = document.querySelectorAll(".langSection");
  let popName = document.querySelector("#popName");
  let popCount = document.querySelector("#popCount");

  let sortedUsers = countries_data.sort((a, b) => b.population - a.population)
  let arr = sortedUsers.slice(0, 10)

//   console.log(filteredUsers);
popBtn.addEventListener("click", function(){
    arr.forEach((countries) => {
        popName.innerHTML += `<ul>
        <li>${countries.name}</li>
        </ul>`
        // console.log(countries.name);
      })
      arr.forEach((popl) => {
        popCount.innerHTML += `<p>${popl.population}</p>`
      })
})